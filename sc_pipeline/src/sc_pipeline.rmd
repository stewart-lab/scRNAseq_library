...
# cat("Create reticulate environment")
...
# use_condaenv("scrnaseq", conda = "/opt/conda/bin/conda", required = TRUE)
...
if (config$process_matrix == TRUE) { # this is only one matrix (raw counts)
  print("processing process_lane2")
  processed_data_list <- future_lapply(config$lanes, process_lane2)
} else if (config$parse == TRUE) { # if this is parse data
  print("processing process_lane")
  processed_data_list <- future_lapply(config$lanes, process_lane, TRUE)
} else { # if this is regular 10x data
  print("processing process_lane")
  processed_data_list <- future_lapply(config$lanes, process_lane, FALSE)
}
file.rename(from = "Rplots.pdf", to = paste0(output_base_dir, "soupX_contamination.pdf"))
```
```{r process_sample}
