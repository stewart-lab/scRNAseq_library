...
def parse_args():
    ...
    group.add_argument(
        "--fastq",
        action="store_true",
        help="Look in the shared_volume directory.",
    )
    args = parser.parse_args()
    return args


def search_gene_full_dirs(search_dir, gene_full_dirs):
...
def main(args):
    ...
    if args.data:
        assert args.data in download_links, (
            "Please specify a correct --data argument."
            + "Use one of the following options: ['REH', 'GAMM_S1', 'GAMM_S2']"
        )
        ...
        subprocess.run(
            ["tar", "-zxvf", f"{args.data}_DATA.tar.gz"], check=True
        )

        # Get the path of the nested directory
        nested_dir = os.path.join(
            data_dir, "dmz", "jfreeman", f"{args.data}_DATA"
        )
        ...
        os.chdir(SCRIPT_DIR)
        gene_full_dirs = search_gene_full_dirs(data_dir, gene_full_dirs)
                            ...
                            if file.endswith(".tsv") or file.endswith(".mtx"):
                                ...
                                if not os.path.exists(
                                    ...
                                    try:
                                        ...
                                        print(
                                            f"Successfully gzipped {file_path}"
                                        )
                                    except Exception as e:
                                        print(
                                            f"Failed to gzip {file_path}: {e}"
                                        )
                                else:
                                    print(
                                        f"{file_path} is already gzipped, skipping."
                                    )
...
def update_config(gene_full_dirs):
...
if __name__ == "__main__":
    args = parse_args()
    genes_full_dirs = main(args)
    update_config(genes_full_dirs)
