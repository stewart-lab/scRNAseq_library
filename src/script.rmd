...
library(jsonlite)
library(rmarkdown)
use_condaenv("gpt4", conda = "/isiseqruns/jfreeman_tmp_home/bin/miniconda3/condabin/conda")
...
# Load Data
```{r}
project_s2_1 <- read_aligned_data("/w5home/bmoore/scRNAseq/GAMM/output_S2-1_mm_mt/Solo.out/GeneFull", "gamm_s2-1")
project_s2_2 <- read_aligned_data("/w5home/bmoore/scRNAseq/GAMM/output_S2-2_mm_mt/Solo.out/GeneFull", "gamm_s2-2")
all_aligned_data <- list(project_s2_1, project_s2_2)
```
...
```{r}
if (config$config$DE_method) == "Seurat" {
  de_results <- find_differentially_expressed_features(clustered_seurat_obj)
} else if (config$config$DE_method) == "Scran" {
