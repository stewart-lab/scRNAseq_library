...
```{r read_in}
human_D205.seurat.train <- readRDS(file = "output_scPred_20230810_093403/human_D205_train_models.rds")
gamm <- readRDS(file = "output_20230711_155556/GAMM_S2_norm.rds")
```
# classify cells
```{r classify_cells}
...
```{r read_in}
gamm <- readRDS(file= "output_scPred_20230911_134940_cc/GAMM_S2_scpred_c0.5.rds")
```

# verify performance with manual annotation
```{r verify_performance}
# add in metadata from previous analysis
metadata.gamm <- read.csv("gamm_manual_annot_metadata_S2_mod.txt", row.names = 1, 
            header = TRUE, sep = "\t") #consolidated marker list: gamm_manual_annot_metadata_c0.5.txt
