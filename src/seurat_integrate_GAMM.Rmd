...
cell.cycle.markers.s <- read.csv2("/w5home/bmoore/scRNAseq_library/cell_cycle_vignette/cell_cycle_orthologs_s.genes.txt", 
    sep = "\t", header = TRUE, row.names = 1)
cell.cycle.markers.g2m <- read.csv2("/w5home/bmoore/scRNAseq_library/cell_cycle_vignette/cell_cycle_orthologs_g2m.genes.txt", 
    sep = "\t", header = TRUE, row.names = 1)
...
# load data
```{r load_data}
gamms2<- readRDS(file = "~/scRNAseq/GAMM/human_data/reh_cellrep_2020/output_seurat_20230718_115135/GAMM_S2_feat_select.rds")
human_D205.seurat<- readRDS(file = "~/scRNAseq/GAMM/human_data/reh_cellrep_2020/output_seurat_20230718_115135/human_D205_feat_select.rds")
```
...
```
```{r save_data}
saveRDS(gamm.combined, file= paste0(output,"gamm.combined_cc.regress.rds"))
```
```{r load_data}
...
```{r}
known.markers <- read.csv2("/w5home/bmoore/scRNAseq_library/DATA/GammLab_Retinal-specific_genelist.txt", sep = "\t", header = TRUE, row.names = 1)
...
```{r}
for (i in levels(gamm.combined)[7:10]){
    print(i)
    # find DE genes
...
# annotate clusters
```{r}
gamm.combined <- RenameIdents(gamm.combined, `1` = "Retinal Prog-Muller Glia", 
    `2` = "Pan PR-BPC", `3` = "Pan PR-BPC",`4` = "BPC", `5` = "Amacrine cell-Synaptic", 
    `6` = "Retinal Prog-Muller Glia", `7` = "none", `8` = "Rod-Neuronal projection", 
    `9` = "BPC", `10` = "none"
    )
